...
const Chatbot = () => {
  const [messages, setMessages] = useState([
    { sender: 'bot', text: 'Hi! Please provide your email so we can assist you further.' },
  ]);
      ...
      try {
        if (step === 'email') {
          ...
          if (userDetails) {
            ...
            setStep('problem');
            setMessages(prevMessages => [...prevMessages, { sender: 'bot', text: 'Thanks! Now, please describe the problem you are facing.' }]);
          } else {
            setMessages(prevMessages => [...prevMessages, { sender: 'bot', text: 'Email not found. Please provide a valid email.' }]);
            setIsTyping(false);
          }
        } else if (step === 'problem' && hasMoreQuestions) {
          const { response, hasNextQuestion, tagline, isUrgent } = await getBotResponse(userInput);
          setMessages(prevMessages => [...prevMessages, { sender: 'bot', text: response }]);
          setHasMoreQuestions(hasNextQuestion);
          if (!hasNextQuestion) {
            setStep('completed');
            await addTicket(tagline, userDetail.id, isUrgent);
            toast({
  ...
  const addTicket = async (problemTagline, userId, isUrgent) => {
    try {
      const response = await fetch(`${BACKEND_URL}/api/ticket/`, {
        ...
        body: JSON.stringify({ ProblemStatement: problemTagline, userId: userId, isUrgent: isUrgent }),
      });
